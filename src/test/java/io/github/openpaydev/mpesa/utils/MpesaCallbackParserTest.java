...
class MpesaCallbackParserTest {
    ...
    void parse_shouldSucceed_forSuccessfulTransaction() throws JsonProcessingException {
        ...
        assertInstanceOf(Double.class, amountItem.getValue(), "Amount should be deserialized as Double.");
        ...
        assertInstanceOf(Long.class, dateItem.getValue(), "Transaction Date should be deserialized as Long.");
        ...
        assertInstanceOf(Long.class, phoneItem.getValue(), "Phone Number should be deserialized as Long.");
    ...
    void parse_shouldSucceed_forFailedTransaction() throws JsonProcessingException {
        ...
        assertNull(stkCallbackData.getCallbackMetadata(), "CallbackMetadata should be null for failed transactions.");
    }

    @Test
    @DisplayName("Should throw JsonProcessingException for malformed JSON")
    void parse_shouldThrowException_forMalformedJson() {
        String malformedJson = new StringBuilder().append("{\n").append("  \"Body\": {\n").append("    \"stkCallback\": {\n").append("      \"ResultCode\": 0,\n").append("    }\n").append("  }\n").append("}").toString();
    ...
    void constructor_shouldBePrivateAndThrowException() throws NoSuchMethodException {
        Constructor<MpesaCallbackParser> constructor = MpesaCallbackParser.class.getDeclaredConstructor();
        assertTrue(java.lang.reflect.Modifier.isPrivate(constructor.getModifiers()), "Constructor must be private.");
