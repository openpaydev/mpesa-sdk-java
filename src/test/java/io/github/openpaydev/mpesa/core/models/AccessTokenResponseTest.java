...
class AccessTokenResponseTest {
    ...
    void shouldDeserializeWithExtraFields() throws JsonProcessingException {
        ...
        AccessTokenResponse accessTokenResponse = objectMapper.readValue(jsonWithExtraField, AccessTokenResponse.class);
    ...
    void toStringShouldContainFieldValues() {
        AccessTokenResponse accessTokenResponse = AccessTokenResponse.builder()
                .accessToken("myTestToken123")
                .expiresIn(3599L)
                .build();

        String stringResponse = accessTokenResponse.toString();

        assertTrue(stringResponse.contains("accessToken=myTestToken123"), "ToString should include the access token.");
        assertTrue(stringResponse.contains("expiresIn=3599"), "ToString should include the expires in value.");
    }
}