...
class StkCallbackTest {
    ...
    void shouldConstructFullObjectGraph() {
        StkCallback.CallbackItem amountItem = StkCallback.CallbackItem.builder()
                .name("Amount")
                .value(1.0)
                .build();

        StkCallback.CallbackItem receiptItem = StkCallback.CallbackItem.builder()
                .name("MpesaReceiptNumber")
                .value("ABC123XYZ")
                .build();

        StkCallback.CallbackMetadata metadata = StkCallback.CallbackMetadata.builder()
                .items(new StkCallback.CallbackItem[]{amountItem, receiptItem})
                .build();

        StkCallback.StkCallbackData callbackData = StkCallback.StkCallbackData.builder()
        ...
        StkCallback.Body body = StkCallback.Body.builder()
                .stkCallback(callbackData)
                .build();

        StkCallback stkCallback = StkCallback.builder()
                .body(body)
                .build();
        ...
        StkCallback.CallbackMetadata retrievedMetadata = stkCallback.getBody().getStkCallback().getCallbackMetadata();
    ...
    void toStringShouldWork() {
        StkCallback.CallbackItem item = StkCallback.CallbackItem.builder().name("Amount").value(100.0).build();
