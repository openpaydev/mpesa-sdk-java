...
class MpesaTokenManagerTest {
  ...
  @Mock private MpesaConfig mpesaConfig;
  ...
  void getAccessToken_inMultiThreadedEnvironment_fetchesTokenOnlyOnce() throws Exception {
    AccessTokenResponse apiResponse =
        AccessTokenResponse.builder().accessToken("threadSafeToken").expiresIn(3599L).build();
        ...
        () -> {
          for (int i = 0; i < futures.size(); i++) {
            ...
            assertEquals(
                "threadSafeToken",
                token,
                "Thread " + i + " should have received the correct token.");
          }
        },
  ...
  void getAccessToken_whenApiReturnsError_throwsMpesaAuthException() {
    ...
    MpesaAuthException exception =
        assertThrows(
            MpesaAuthException.class,
  ...
  /**
   * NEW TEST: This test simulates a complete network failure. It verifies that the raw IOException
   * from OkHttp is correctly caught and wrapped inside our custom MpesaAuthException.
   */
  @Test
  ...
  void getAccessToken_whenNetworkFails_throwsMpesaAuthExceptionWrappingIOException() {
    ...
    assertNotNull(
        exception.getCause(), "The cause of the exception should be the original IOException.");
    assertInstanceOf(
        IOException.class, exception.getCause(), "The cause should be an instance of IOException.");
  }
  ...
  void getAccessToken_whenNoToken_fetchesNewTokenSuccessfully() throws Exception {
    AccessTokenResponse apiResponse =
        AccessTokenResponse.builder().accessToken("aNewToken").expiresIn(3599L).build();
    mockWebServer.enqueue(
        new MockResponse()
            .setResponseCode(200)
            .setBody(objectMapper.writeValueAsString(apiResponse)));
  ...
  void getAccessToken_whenTokenIsValid_returnsCachedTokenWithoutApiCall() throws Exception {
    AccessTokenResponse apiResponse =
        AccessTokenResponse.builder().accessToken("aValidCachedToken").expiresIn(3599L).build();
    mockWebServer.enqueue(
        new MockResponse()
            .setResponseCode(200)
            .setBody(objectMapper.writeValueAsString(apiResponse)));
  ...
  void getAccessToken_whenTokenIsExpired_fetchesNewToken() throws Exception {
    AccessTokenResponse initialResponse =
        AccessTokenResponse.builder().accessToken("expiredToken").expiresIn(3600L).build();
    mockWebServer.enqueue(
        new MockResponse()
            .setResponseCode(200)
            .setBody(objectMapper.writeValueAsString(initialResponse)));
    ...
    AccessTokenResponse refreshResponse =
        AccessTokenResponse.builder().accessToken("refreshedToken").expiresIn(3600L).build();
    mockWebServer.enqueue(
        new MockResponse()
            .setResponseCode(200)
    ...
    assertEquals(
        2,
        mockWebServer.getRequestCount(),
        "API should be called a second time to refresh the token.");
  }
}
