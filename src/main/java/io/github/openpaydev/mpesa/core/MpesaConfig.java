 ...
 *
 * <p>This class holds all the necessary credentials and settings required to interact with the
 * Mpesa API. It is immutable and should be created once and reused throughout your application.
 *
 * <p>An instance can be created using the builder pattern or from environment variables.
 *
 * <p>Example using the builder:
 *
 * <pre>{@code
...
public class MpesaConfig {
  ...
  /**
   * The Business Short Code, which is the identifier for your PayBill or Till Number. Renamed from
   * 'shortCode' to align with the official M-Pesa API documentation.
   */
  ...
  /**
   * The Mpesa environment to use, either {@link MpesaEnvironment#SANDBOX} or {@link
   ...
   *
   * <p>This is the recommended way to configure the SDK in a production environment. It uses {@code
   * System.getenv()} as the source for the variables.
   *
  ...
  /**
   * Creates a {@link MpesaConfig} instance using a provided function to look up environment
   * variables.
   *
   * <p>This method is designed for testability, allowing you to inject a fake environment.
   *
   * @param envProvider A function that takes an environment variable name (String) and returns its
   *     value (String).
   * @return A new {@link MpesaConfig} instance.
   */
