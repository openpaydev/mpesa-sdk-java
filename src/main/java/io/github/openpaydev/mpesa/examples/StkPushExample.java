 ...
 * <p><b>Prerequisites:</b>
 *
 * <p>Before running this example, you must set the following environment variables:
 *
 * <ul>
 *   <li>{@code MPESA_CONSUMER_KEY}: Your sandbox consumer key.
 *   <li>{@code MPESA_CONSUMER_SECRET}: Your sandbox consumer secret.
 *   <li>{@code MPESA_PASSKEY}: Your sandbox Lipa Na M-Pesa passkey.
 *   <li>{@code MPESA_SHORTCODE}: The sandbox business shortcode (e.g., "174379").
 * </ul>
 */
public class StkPushExample {
    ...
    try {
      ...
      StkPushRequest request =
          StkPushRequest.newPayBillRequest(
              ...
              "https://mydomain.com/path" // TODO: MUST be a public HTTPS URL. Use ngrok for local
              // testing.
              );
      ...
      System.out.println(
          "[INFO] Querying final status for CheckoutRequestID: " + checkoutRequestID);
  ...
  private static void validateConfig(MpesaConfig config) {
    if (config.getConsumerKey() == null
        || config.getConsumerSecret() == null
        || config.getPassKey() == null
        || config.getBusinessShortCode() == null) {
